...
public class ValidateEmailRepository {

    private MutableLiveData<ChangeEmailResponse> mutableLiveData;


    public ValidateEmailRepository() {

        mutableLiveData = new MutableLiveData<>();
    }

    //  validate email
    public void validateEmail(@NonNull String token, @NonNull String email, @NonNull String otp){
        ApiDataService apiDataService = RetrofitInstance.getInstance().create(ApiDataService.class);
        Call<ChangeEmailResponse> call = apiDataService.emailOtp("application/json","Bearer "+token,email,otp);
